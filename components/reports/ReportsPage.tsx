...
export function ReportsPage() {
  ...
  let reports = isAdmin ? data.reports : data.reports.filter((r) => r.entityId === session?.entityId || r.regionId === session?.regionId);
            ...
            render: (r) => {
              ...
              return (
                <div className="flex gap-1">
                  <Button variant="ghost" size="icon" onClick={() => setPreview(report)}><Eye className="h-4 w-4" /></Button>
                  <Button variant="ghost" size="icon" onClick={() => toast.success(`Downloading ${report.format} report (demo)`)}><Download className="h-4 w-4" /></Button>
                  {isAdmin && <Button variant="ghost" size="icon" onClick={() => { deleteReport(report.id); toast.success("Report deleted"); }}><Trash2 className="h-4 w-4 text-red-500" /></Button>}
                </div>
              );
            },
          },
