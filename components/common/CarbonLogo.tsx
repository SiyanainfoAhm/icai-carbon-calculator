...
export function CarbonLogo({ size = "md", showText = true, className }: LogoProps) {
  ...
  return (
    <div className={cn("flex items-center gap-3", className)}>
      <div className={cn("relative flex items-center justify-center rounded-xl bg-gradient-to-br from-teal-500 to-emerald-600 shadow-lg shadow-teal-200", sizes[size])}>
      ...
      {showText && (
        <div>
          ...
          <p className="text-[10px] font-medium text-teal-600 tracking-wide uppercase">Carbon Calculator</p>
        </div>
      )}
    </div>
  );
